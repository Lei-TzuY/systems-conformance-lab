...
from .http_content_encoding_adapter import (
    DEFAULT_MAX_OBSERVED_BODY_BYTES,
    DEFAULT_MAX_WIRE_BODY_BYTES,
...
const MAX_WIRE_BODY_BYTES = __MAX_WIRE_BODY_BYTES__;
...
function parseCase(raw) {
  if (raw.length < CASE_HEADER_BYTES) {
    return { error: "protocol_error", ok: false };
  }
  ...
  const declaredLength = raw.readUInt32BE(1);
  if (declaredLength > MAX_WIRE_BODY_BYTES) {
    return { error: "wire_body_too_large", ok: false };
  }
  if (raw.length !== CASE_HEADER_BYTES + declaredLength) {
    return { error: "protocol_error", ok: false };
  }

  return {
    body: raw.subarray(CASE_HEADER_BYTES),
    contentEncoding: mode === 1 ? "gzip" : null,
  };
}
...
async function fetchLoopback(parsed) {
  const server = http.createServer((request, response) => {
    ...
    response.setHeader("Content-Length", String(parsed.body.length));
    if (parsed.contentEncoding !== null) {
      response.setHeader("Content-Encoding", parsed.contentEncoding);
    }
...
async function main() {
  if (
    ...
    process.stderr.write("http_content_encoding_runtime_identity_mismatch\n");
    process.exitCode = 3;
    return;
  }

  const raw = await readBoundedStdin(CASE_HEADER_BYTES + MAX_WIRE_BODY_BYTES);
  if (raw === null) {
    emit({ error: "case_too_large", ok: false });
    return;
  }
...
main().catch(() => {
  process.stderr.write("http_content_encoding_worker_unexpected_error\n");
  process.exitCode = 4;
});
""".strip()
...
def _probe_node_runtime_identity(node_executable: str) -> tuple[str, str, str]:
    ...
    if (
        ...
        raise RuntimeError("unable to probe Node HTTP runtime identity")

    try:
        payload = json.loads(result.stdout.text)
    except (json.JSONDecodeError, RecursionError) as exc:
        raise RuntimeError("Node HTTP runtime identity probe returned invalid JSON") from exc

    if not isinstance(payload, dict) or set(payload) != {"node", "undici", "zlib"}:
        raise RuntimeError("Node HTTP runtime identity probe returned invalid fields")
    ...
    if any(
        ...
        raise RuntimeError("Node HTTP runtime identity probe returned invalid values")
    return values


def _node_script(
    *,
    max_wire_body_bytes: int,
    max_observed_body_bytes: int,
    runtime_identity: tuple[str, str, str],
) -> str:
    node_version, undici_version, zlib_version = runtime_identity
    return (
        _NODE_SCRIPT.replace("__MAX_WIRE_BODY_BYTES__", str(max_wire_body_bytes))
...
class HTTPContentEncodingNodeTarget:
    """Node fetch loopback HTTP response-decoding target."""

    max_wire_body_bytes: int = DEFAULT_MAX_WIRE_BODY_BYTES
    ...
    def __post_init__(self) -> None:
        _validate_budget("max_wire_body_bytes", self.max_wire_body_bytes)
        ...
        if which(self.node_executable) is None:
            raise RuntimeError(
                "Node runtime is required for HTTPContentEncodingNodeTarget: "
    ...
    def as_command_target(self) -> CommandTarget:
        return CommandTarget(
            (
                ...
                "-e",
                _node_script(
                    max_wire_body_bytes=self.max_wire_body_bytes,
