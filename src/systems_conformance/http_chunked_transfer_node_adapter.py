...
from .http_chunked_transfer_adapter import (
...
async function fetchLoopback(transferBody) {
  ...
  const server = net.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      if (request.includes(Buffer.from("\r\n\r\n", "ascii"))) {
        socket.end(Buffer.concat([responsePrefix, transferBody]));
      }
    });
  });
...
async function main() {
  if (
    ...
    process.stderr.write("http_chunked_transfer_runtime_identity_mismatch\n");
    process.exitCode = 3;
    return;
  }

  const transferBody = await readBoundedStdin(MAX_TRANSFER_BODY_BYTES);
  if (transferBody === null) {
    emit({ error: "transfer_body_too_large", ok: false });
    return;
  }

  await fetchLoopback(transferBody);
}

main().catch(() => {
  process.stderr.write("http_chunked_transfer_worker_unexpected_error\n");
  process.exitCode = 4;
});
""".strip()
...
def _probe_node_runtime_identity(node_executable: str) -> tuple[str, str]:
    ...
    if (
        ...
        raise RuntimeError("unable to probe Node HTTP chunked-transfer runtime identity")

    try:
        payload = json.loads(result.stdout.text)
    except (json.JSONDecodeError, RecursionError) as exc:
        raise RuntimeError(
            "Node HTTP chunked-transfer runtime identity probe returned invalid JSON"
        ) from exc

    if not isinstance(payload, dict) or set(payload) != {"node", "undici"}:
        raise RuntimeError(
            "Node HTTP chunked-transfer runtime identity probe returned invalid fields"
        )

    values = payload["node"], payload["undici"]
    if any(
        not isinstance(value, str) or not value or len(value) > 128
        for value in values
    ):
        raise RuntimeError(
            "Node HTTP chunked-transfer runtime identity probe returned invalid values"
        )
    return values
...
class HTTPChunkedTransferNodeTarget:
    """Node Fetch loopback HTTP chunked-transfer observation target."""
    ...
    def __post_init__(self) -> None:
        ...
        if which(self.node_executable) is None:
            raise RuntimeError(
                "Node runtime is required for HTTPChunkedTransferNodeTarget: "
