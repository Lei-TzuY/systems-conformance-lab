...
if (process.version !== EXPECTED_NODE_VERSION) {
  process.stderr.write("url_parser_runtime_identity_mismatch\n");
  process.exitCode = 3;
} else {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      ...
      process.stderr.write("url_parser_node_runtime_error\n");
      process.exitCode = 3;
    }
  }
  ...
  if (text !== undefined) {
    try {
      ...
      if (
        ...
        throw new TypeError("out-of-scope URL");
      }
      emit({
...
def _probe_node_runtime_identity(node_executable: str) -> tuple[str]:
    ...
    if (
        ...
        raise RuntimeError("unable to probe Node URL parser runtime identity")

    try:
        payload = json.loads(result.stdout.text)
    except (json.JSONDecodeError, RecursionError) as exc:
        raise RuntimeError("Node URL parser runtime identity probe returned invalid JSON") from exc

    if not isinstance(payload, dict) or set(payload) != {"node"}:
        raise RuntimeError("Node URL parser runtime identity probe returned invalid fields")
    node_version = payload["node"]
    if not isinstance(node_version, str) or not node_version or len(node_version) > 128:
        raise RuntimeError("Node URL parser runtime identity probe returned invalid value")
    return (node_version,)
...
class URLNodeParseTarget:
    """Node WHATWG URL target for absolute HTTP/HTTPS parser interoperability."""
    ...
    def __post_init__(self) -> None:
        ...
        if which(self.node_executable) is None:
            raise RuntimeError(
                f"Node runtime is required for URLNodeParseTarget: {self.node_executable}"
            )
        object.__setattr__(
            self,
            "_runtime_identity",
