...
if (process.version !== EXPECTED_NODE_VERSION) {
  process.stderr.write("url_query_runtime_identity_mismatch\n");
  process.exitCode = 3;
} else {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      ...
      process.stderr.write("url_query_node_runtime_error\n");
      process.exitCode = 3;
    }
  }
...
def _probe_node_runtime_identity(node_executable: str) -> tuple[str]:
    ...
    if (
        ...
        raise RuntimeError("unable to probe Node URL query runtime identity")

    try:
        payload = json.loads(result.stdout.text)
    except (json.JSONDecodeError, RecursionError) as exc:
        raise RuntimeError(
            "Node URL query runtime identity probe returned invalid JSON"
        ) from exc

    if not isinstance(payload, dict) or set(payload) != {"node"}:
        raise RuntimeError("Node URL query runtime identity probe returned invalid fields")
    node_version = payload["node"]
    if not isinstance(node_version, str) or not node_version or len(node_version) > 128:
        raise RuntimeError("Node URL query runtime identity probe returned invalid value")
    return (node_version,)
...
class URLNodeQueryCanonicalizationTarget:
    """Node WHATWG URL + URLSearchParams full-URL query canonicalization target."""
    ...
    def __post_init__(self) -> None:
        ...
        if which(self.node_executable) is None:
            raise RuntimeError(
                "Node runtime is required for URLNodeQueryCanonicalizationTarget: "
