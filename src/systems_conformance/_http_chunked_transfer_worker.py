...
def _read_bounded_case(max_transfer_body_bytes: int) -> bytes | None:
    raw = sys.stdin.buffer.read(max_transfer_body_bytes + 1)
    if len(raw) > max_transfer_body_bytes:
        return None
    return raw
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not _runtime_identity_matches(args):
        sys.stderr.write("http_chunked_transfer_runtime_identity_mismatch\n")
        return 3

    transfer_body = _read_bounded_case(args.max_transfer_body_bytes)
