...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=DEFAULT_MAX_WIRE_BODY_BYTES,
    )
    parser.add_argument(
...
def _read_bounded_case(max_wire_body_bytes: int) -> bytes | None:
    limit = _CASE_HEADER_BYTES + max_wire_body_bytes
...
) -> tuple[str | None, bytes] | dict[str, object]:
    if len(raw) < _CASE_HEADER_BYTES:
        return {"error": "protocol_error", "ok": False}

    mode = raw[0]
    if mode not in (_IDENTITY, _GZIP):
        return {"error": "protocol_error", "ok": False}

    declared_length = int.from_bytes(raw[1:5], "big")
    if declared_length > max_wire_body_bytes:
        return {"error": "wire_body_too_large", "ok": False}
    if len(raw) != _CASE_HEADER_BYTES + declared_length:
        return {"error": "protocol_error", "ok": False}

    content_encoding = "gzip" if mode == _GZIP else None
    return content_encoding, raw[_CASE_HEADER_BYTES:]
    ...
    try:
        ...
        with connection:
            ...
            if content_encoding is not None:
                headers.append(f"Content-Encoding: {content_encoding}".encode("ascii"))
    ...
    *,
    ...
    try:
        with urlopen(f"http://127.0.0.1:{port}/", timeout=5.0) as response:
            ...
            observed_encoding = response.headers.get("Content-Encoding")
            observed_length = response.headers.get("Content-Length")
    ...
    if not isinstance(observed_length, str):
        return {"error": "http_fetch_error", "ok": False}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not _runtime_identity_matches(args):
        sys.stderr.write("http_content_encoding_runtime_identity_mismatch\n")
        return 3

    raw = _read_bounded_case(args.max_wire_body_bytes)
    if raw is None:
        output = {"error": "case_too_large", "ok": False}
    else:
        parsed = _parse_case(raw, args.max_wire_body_bytes)
        if isinstance(parsed, dict):
            output = parsed
        else:
            content_encoding, body = parsed
