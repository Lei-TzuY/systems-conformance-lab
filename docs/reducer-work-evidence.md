...
Evidence-aware repro loading validates the reserved record before replay. It requires the exact v1 fields, non-negative integer work counters, at least the initial predicate evaluation, and accepted-step counts bounded by evaluated and visited candidates. It also enforces the reducer's structural accounting invariant that every predicate evaluation after the initial case must correspond to a visited candidate, so `candidate_visits` can never be smaller than `evaluations - 1`. Finally, `termination_reason` must agree with `exhausted_budget`. Legacy and caller-defined metadata without the reserved key remain valid. This makes transported or archived reducer evidence fail closed on schema or semantic drift instead of merely preserving an untrusted namespaced object.
