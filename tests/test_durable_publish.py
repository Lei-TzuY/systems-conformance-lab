...
def _publisher(fault_stage: str | None = None) -> FaultingDurableFilePublisher:
...
@pytest.mark.skipif(os.name == "nt", reason="directory fsync is not supported on Windows")
def test_durable_publisher_completes_real_durability_sequence(tmp_path: Path) -> None:
    destination = tmp_path / "live.bin"
    source = tmp_path / "stage.bin"
    destination.write_bytes(b"old")

    _publisher().publish(source, destination, b"new")
