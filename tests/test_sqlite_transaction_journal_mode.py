...
def test_sqlite_transaction_target_rejects_unknown_journal_mode() -> None:
    with pytest.raises(ValueError, match="journal_mode must be 'delete' or 'wal'"):
        SQLiteTransactionTarget(journal_mode="memory")  # type: ignore[arg-type]


def test_wal_transaction_persists_committed_rows_across_reopen() -> None:
...
def test_wal_and_delete_journals_have_matching_reopen_semantics(finalize: str) -> None:
