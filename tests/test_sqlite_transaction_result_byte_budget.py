...
def test_transaction_target_rejects_invalid_result_byte_budgets(value: object) -> None:
    ...
    with pytest.raises(
        ValueError, match="max_transcript_result_bytes must be a positive integer"
    ):
        SQLiteTransactionTarget(max_transcript_result_bytes=value)  # type: ignore[arg-type]
...
def test_transaction_transcript_result_budget_accepts_exact_boundary() -> None:
    result = _execute(
        _request("SELECT 1"),
        SQLiteTransactionTarget(max_transcript_result_bytes=60),
    )
    assert result.infrastructure_error is None
    assert result.exit_code == 0
    payload = json.loads(result.stdout.text)
    assert payload["transaction"][0]["rows"] == [[1]]
    assert payload["observation"]["rows"] == [[1]]


def test_transaction_transcript_result_budget_rejects_cumulative_small_results() -> None:
    result = _execute(
        _request("SELECT 1"),
        SQLiteTransactionTarget(max_transcript_result_bytes=59),
    )
    assert result.infrastructure_error is None
    assert result.exit_code == 4
    assert result.stdout.text == ""
    assert result.stderr.text.strip() == (
        "result_error: transaction transcript results exceed "
        "max_transcript_result_bytes: 59"
    )


def test_transaction_transcript_result_budget_is_part_of_replay_identity() -> None:
    low = SQLiteTransactionTarget(max_transcript_result_bytes=59).as_command_target()
    high = SQLiteTransactionTarget(max_transcript_result_bytes=60).as_command_target()
    assert low.argv != high.argv
    assert "--max-transcript-result-bytes" in low.argv
...
def test_real_harness_observes_transcript_budget_as_product_mismatch() -> None:
    candidate = SQLiteTransactionTarget(max_transcript_result_bytes=59).as_command_target()
    oracle = SQLiteTransactionTarget(max_transcript_result_bytes=60).as_command_target()
    harness = DifferentialHarness(candidate=candidate, oracle=oracle, timeout_seconds=2.0)
    run = harness.evaluate(_request("SELECT 1"))
    assert run.candidate.infrastructure_error is None
    assert run.oracle.infrastructure_error is None
    assert run.candidate.exit_code == 4
    assert run.oracle.exit_code == 0
    ...
    assert json.loads(run.oracle.stdout.text)["observation"]["rows"] == [[1]]
    assert run.comparison.classification == "product_mismatch"
    assert run.signature is not None
    assert run.signature.kind == "product_mismatch"
