...
def _request(transaction_value: str, observe_value: str | None = None) -> bytes:
    observe = (
        {"sql": "SELECT 1", "params": []}
        if observe_value is None
        else {"sql": "SELECT ?", "params": [observe_value]}
    )
    return json.dumps(
        {
            "setup": [],
            "transaction": [{"sql": "SELECT ?", "params": [transaction_value]}],
            "observe": observe,
        },
        ensure_ascii=False,
        separators=(",", ":"),
...
def test_sqlite_transaction_param_value_budget_allows_exact_utf8_boundary() -> None:
    result = _execute("éé", max_param_value_bytes=4)

    assert result.infrastructure_error is None
    assert result.exit_code == 0
    assert json.loads(result.stdout.text) == {
        "transaction": [{"columns": ["?"], "rows": [["éé"]]}],
        "observation": {"columns": ["1"], "rows": [[1]]},
    }


def test_sqlite_transaction_param_value_budget_rejects_next_utf8_byte_before_execution() -> None:
    ...
    assert result.stderr.text.strip() == (
        "protocol_error: transaction param value exceeds max_param_value_bytes: 3"
    )


def test_sqlite_transaction_param_value_budget_applies_to_observation() -> None:
    result = _execute("ok", max_param_value_bytes=3, observe_value="éé")

    assert result.infrastructure_error is None
    assert result.exit_code == 2
    assert result.stdout.text == ""
    assert result.stderr.text.strip() == (
        "protocol_error: observe param value exceeds max_param_value_bytes: 3"
    )


@pytest.mark.parametrize("max_param_value_bytes", [0, -1, True, 1.5, "10"])
def test_sqlite_transaction_target_rejects_invalid_param_value_budgets(
...
def test_sqlite_transaction_param_value_budget_changes_replay_context() -> None:
    candidate = SQLiteTransactionTarget(max_param_value_bytes=3).as_command_target()
    oracle = SQLiteTransactionTarget(max_param_value_bytes=4).as_command_target()
...
def test_real_differential_harness_observes_sqlite_transaction_param_value_budget() -> None:
    candidate = SQLiteTransactionTarget(max_param_value_bytes=3).as_command_target()
    oracle = SQLiteTransactionTarget(max_param_value_bytes=4).as_command_target()
