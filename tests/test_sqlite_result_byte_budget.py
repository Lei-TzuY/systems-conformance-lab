...
def test_sqlite_target_rejects_invalid_result_byte_budgets(value: object) -> None:
...
def test_sqlite_result_byte_budget_accepts_exact_rows_json_boundary() -> None:
    result = _execute(
        ...
        SQLiteQueryTarget(max_result_bytes=9),
    )

    assert result.infrastructure_error is None
    assert result.exit_code == 0
    assert json.loads(result.stdout.text)["rows"] == [[1], [2]]


def test_sqlite_result_byte_budget_rejects_before_full_result_serialization() -> None:
    result = _execute(
        _request("SELECT 1 UNION ALL SELECT 2"),
        SQLiteQueryTarget(max_result_bytes=8),
    )
...
def test_result_byte_budget_is_part_of_replay_identity() -> None:
    low = SQLiteQueryTarget(max_result_bytes=8).as_command_target()
    high = SQLiteQueryTarget(max_result_bytes=9).as_command_target()
